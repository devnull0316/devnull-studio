...
# 周辺・波及情報（関連が薄くても拾う）
直接の可用性変化でなくても、本件の理解や今後の予測に資する情報は拾って報告する。
各項目に「関連度: 高/中/低」を付すこと。例:
- 輸出管理規制の他モデル・他社・他国への波及や前例化の議論
- 議会・訴訟・規制当局・業界団体の動き
- データ保持(30日)・Covered Model 指定・Project Glasswing 関連の変化
- 復旧/再開に向けた非公式情報・リーク・観測（憶測として明示）
- 株価・契約・顧客離脱など事業面の影響
- 技術コミュニティ(Hacker News等)での新たな論点
※「面白いが関連は薄い」程度でも、低ラベルを付けて1行で残してよい。
...
- 周辺情報は拾うが、確実情報と混同させない（必ず別セクション＋関連度ラベル）。
...
4. 【周辺・参考情報】関連度ラベル(高/中/低)付き。薄くても拾う
5. 【推奨アクション】依存ユーザー/開発者が今取るべき具体策
6. 【次回確認の着目点】
```
